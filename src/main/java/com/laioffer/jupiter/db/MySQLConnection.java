...
public class MySQLConnection {
    ...
    public MySQLConnection() throws MySQLException {
        try {
            ...
            Class.forName("com.mysql.cj.jdbc.Driver").newInstance(); //反射机制
            //Connect to database
    ...
    public void close() {
        if (conn != null) {
            try {
                conn.close();//disconnect the database (benefit:Memory resources can be recycled)释放资源，提高效率
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
    ...
    public void setFavoriteItem(String userId, Item item) throws MySQLException {
        ...
        //INSERT USERID and ITEM ID favorite table, maybe insert item into item table?
        saveItem(item);
